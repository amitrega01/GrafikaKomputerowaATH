#include <windows.h>
#include <cmath>// plik nagłówkowy dla biblioteki matematycznej
#include <iostream>
#include <cstdio> // plik nagłówkowy dla we/wy standardowego
#include <cstdlib> // plik nagłówkowy dla bibioteki standardowej
#include "glu.h" // plik nagłówkowy dla biblioteki GLu32
...
typedef struct bpatch { // struktura dla wielomiana fragmentu Beziera 3 stopnia
	POINT_3D anchors[4][6]; // siatka 4x7 
...
int divs = 2; // number of interpolation (distribution of polygon)
...
void initBezier(void) {
	for (int i = 0; i < 4; i++)
		for (int p = 0; p <7; p++) {
			...
			//mybezier.anchors[i][p].x = mybezier.anchors[i][p].x*sin(6*0.0001 * t++);
			mybezier.anchors[i][p].y = mybezier.anchors[i][p].y*sin(6*0.0001  * t++);
			mybezier.anchors[i][p].z = mybezier.anchors[i][p].z*sin(6 * t++);
		}
